...
test("should clicking button increments counter", () => {
  ...
  expect(counterDisplay.text()).toContain(
    `The counter is currently ${counter + 1}`
  );
});

test("should clicking button decrements counter", () => {
  const counter = 5;
  const wrapper = setup(null, { counter });
...
test("should counter doesnt allow be below than 0", () => {
  const state = {
    counter: 0,
    isZero: true,
  };
...
test("should error remove when increment button is clicked", () => {
  const state = {
    counter: 0,
    isZero: true,
  };
